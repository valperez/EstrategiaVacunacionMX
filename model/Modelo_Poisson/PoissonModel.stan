...
data {
  ...
  int<lower=0> P_pais_2[2, ndias_isr];           // vector de hosp o muertos de Israel (solo tiene un renglon porque no tenemos edades)
...
model {
  ...
  for (t in (m + 1):ndias){
    ...
    for (k in 1:m){
      ln_p_edad = ln_p_edad + lambda[k]*to_vector(P_edades[1:(nedades*npaises),t-k]) ./ to_vector(P_poblacion); //FIXME
    }
...
generated quantities {
  ...
  vector[nedades*npaises] ln_p_edad;
  int EdadPred2[1, dias_predict + ndias];
  vector[1] ln_p_edad2;       //CHECKME No estoy segura de las dimensiones
  ...
  for (t in (m + 1):ndias){
    ln_p_edad = alpha + cos(2*pi()*(t + 50)/180) * beta_yearly_cosine +
                         sin(2*pi()*(t + 50)/180) * beta_yearly_sine;
    for (k in 1:m){
      ln_p_edad = ln_p_edad + lambda[k]* to_vector(P_edades[1:(nedades*npaises),t-k]) ./ to_vector(P_poblacion); 
    }
  ...
  for (t in (m + 1):ndias_isr){
    ln_p_edad2 = alpha + gamma[t]*vacunados[t] + cos(2*pi()*(t + 50)/180) * beta_yearly_cosine + 
                                            sin(2*pi()*(t + 50)/180) * beta_yearly_sine;
    for (k in 1:m){
      ln_p_edad2 = ln_p_edad2 + lambda[k]* P_pais_2[1,t-k] / pob_total_isr; //FIXME
    }
    /*for (j in 1:(nedades*npaises)){
  ...
  for (t in (ndias_isr + 1):(ndias_isr + m)){ 
    ln_p_edad2 = alpha*gamma[t]*vacunados[t] + cos(2*pi()*(t + 50)/180) * beta_yearly_cosine + 
                         sin(2*pi()*(t + 50)/180) * beta_yearly_sine;
    for (k in 1:m){
      if (t - k <= ndias_isr){
        ln_p_edad2 = ln_p_edad2 + lambda[k]*P_pais_2[1,t-k] / pob_total_isr; 
      } else { 
        ln_p_edad2 = ln_p_edad2 + lambda[k]*EdadPred2[1,t-k] / pob_total_isr; 
      }
    }
  ...
  for (t in (ndias_isr + m + 1):(ndias_isr +  dias_predict)){
    ln_p_edad2 = alpha*gamma[t]*vacunados[t] + cos(2*pi()*(t + 50)/180) * beta_yearly_cosine + 
          sin(2*pi()*(t + 50)/180) * beta_yearly_sine;
    for (k in 1:m){
      ln_p_edad2 = ln_p_edad2 + lambda[k]* EdadPred2[1, t - k] / pob_total_isr;
